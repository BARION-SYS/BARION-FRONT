...
- **Colores del panel (primario + fondo): preferencia de CADA PERSONA y ADAPTATIVOS.** Los cambia **cualquiera que use el panel** —dueño, barbero, recepción y también el staff de plataforma en `/admin`—: no hay rol ni permiso que los proteja porque no hay nada que proteger. Viven en `store/marca.store.ts` con `persist`, o sea en el **localStorage de ese navegador**: no viajan a la api, no son dato del negocio, no hay petición que esperar y **cambiarlos no le mueve la pantalla a nadie más** — cada quien pone el panel como le gusta. Se eligen en el `BrandStudio` (botón de paleta del navbar → Modal con preview del diseño; presets en `config/marca.ts` + selector libre), con botón Aplicar — nada cambia hasta aplicar. `TenantProvider` inyecta un `<style>` con `cssDeMarca` (`shared/utils/color`): un bloque `:root` y otro `.dark` usando **variantes adaptativas** del color elegido (`variantesPrimario`/`variantesFondo` ajustan luminosidad por tema, foreground por contraste) — la estética se conserva en claro y oscuro. Sin elección mandan los defaults de globals.css. Configuración → Apariencia NO configura colores: es logotipo + vista previa del portal.
- **El portal público (`/b/[slug]`) es el caso distinto: ahí la marca SÍ es del negocio y llega de la api** con la ficha de la barbería (`GET /publico/barberias/:slug`), que las páginas de `/b/[slug]` escriben en ese mismo store para teñir la vista. El cliente que reserva SOLO la ve, jamás la elige: es la identidad de la barbería, no su gusto.
...
- **Primitivos UI → shadcn** (`shared/components/ui/`, instalados con `pnpm dlx shadcn@latest add x`). Nunca construir botones/inputs/dialogs desde cero. Nativos solo sin pieza shadcn (grilla de calendario, svg del QR). **Regla dura, sin excepción por comodidad**: `<select>`, `<textarea>`, `<button>` de formulario nunca en HTML nativo — son `Select`, `Textarea`, `Button` de shadcn, aunque la pieza ya exista en el repo y solo haga falta importarla. Si `pnpm dlx shadcn@latest add textarea` nunca se corrió, se corre; no es excusa para escribir el elemento a mano con clases de Tailwind copiadas de `Input`.
- **`Select`/`Checkbox`/`Switch` van con `Controller` de RHF, nunca con `register` directo.** `register` exige una ref de elemento nativo; los primitivos de `shared/components/ui/` envuelven `@base-ui/react`, que no expone esa ref — de ahí que el atajo real (no la excepción válida) sea caer a un `<select>` nativo para poder seguir usando `register`. Patrón correcto: `<Controller control={control} name="x" render={({ field }) => <Select value={field.value} onValueChange={field.onChange}>...</Select>} />` (ver `features/citas/components/CitasForm.tsx`, `CampoSelect`).
...
**UI**: errores sin `getErrorMessage` · toast para error de validación de campo (va inline) · primitivos UI a mano (`<select>`, `<textarea>`, `<button>` de formulario nativos — usar `Select`/`Textarea`/`Button` de shadcn con `Controller` cuando `register` no aplique) · color hardcodeado o `dark:` · formateo inline (`toLocaleString`/`toFixed`/`Intl.*`) fuera de `shared/utils` · lógica de tema fuera de `useTheme()` · UI no responsive (`100vh`, anchos fijos, scroll horizontal) · **spinner o texto "Cargando…" para cargar datos — SIEMPRE `DataSkeleton`/`Loadable`**.
