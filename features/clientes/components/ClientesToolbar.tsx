  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          { etiqueta: "Total clientes", valor: resumen.totalClientes },
          { etiqueta: "Nuevos hoy", valor: resumen.nuevosHoy },
        ].map((s) => (
          <Card key={s.etiqueta} className="gap-0 p-2.5 text-center">
            <p className="text-lg font-bold text-foreground tabular-nums">{s.valor}</p>
            <p className="text-[10px] text-muted-foreground">{s.etiqueta}</p>
          </Card>
        ))}
      </div>
    </div>
