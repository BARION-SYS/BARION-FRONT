...
/**
 * Excepciones de UNA persona sobre lo que le da su rol.
 *
...
export function RolesExcepcionesForm({
  ...
  const alternar = (clave: string) => {
    setCambios((previos) => {
      ...
      const actual = estadoDe(clave)
      // El ciclo depende de si su rol ya lo trae: sobre algo heredado lo útil es
      // quitarlo; sobre algo que no tiene, dárselo.
      if (actual === "hereda") {
        siguiente.set(clave, !delRol.has(clave))
      } else if (delRol.has(clave) ? actual === "revocado" : actual === "concedido") {
